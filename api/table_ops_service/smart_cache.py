...
def _ensure_csv_entries_table(conn):
    """Create the VEDA_WORKSHEET_CSVS table if it doesn't exist."""
    try:
        cur = conn.cursor()
        cur.execute("""
            SELECT table_name FROM user_tables WHERE table_name = 'VEDA_WORKSHEET_CSVS'
        """)
        row = cur.fetchone()
        if not row:
            cur.execute("""
                CREATE TABLE VEDA_WORKSHEET_CSVS (
                  ID             NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY,
                  FILE_NAME      VARCHAR2(512),
                  STORED_PATH    VARCHAR2(1024),
                  ORIGINAL_NAME  VARCHAR2(512),
                  CREATED_AT     TIMESTAMP DEFAULT SYSTIMESTAMP,
                  CONSTRAINT VEDA_WORKSHEET_CSVS_PK PRIMARY KEY (ID)
                )
            """)
            conn.commit()
    except Exception as e:
        app.logger.warning(f"Ensure CSV entries table failed: {e}")
...
def _serialize_csv_entry(row):
    if not row:
        return None
    entry_id, file_name, stored_path, original_name, created_at = row
    return {
...
@app.post('/table/csv_entries')
def table_csv_entry_create():
    body = request.get_json(force=True) or {}
    file_name = (body.get('fileName') or '').strip()
    stored_path = (body.get('storedPath') or '').strip()
    original_name = (body.get('originalName') or '').strip() or file_name
    if not stored_path:
        return jsonify({ 'error': 'storedPath required' }), 400
    try:
        conn = _oracle_connect()
    except Exception as e:
        return jsonify({ 'error': f'Oracle connect failed: {e}' }), 500
    try:
        _ensure_csv_entries_table(conn)
        cur = conn.cursor()
        id_var = cur.var(oracledb.NUMBER)
        cur.execute(
            """
            INSERT INTO VEDA_WORKSHEET_CSVS (FILE_NAME, STORED_PATH, ORIGINAL_NAME)
            VALUES (:file_name, :stored_path, :original_name)
        ...
        conn.commit()
        cur.execute(
            "SELECT ID, FILE_NAME, STORED_PATH, ORIGINAL_NAME, CREATED_AT FROM VEDA_WORKSHEET_CSVS WHERE ID = :id",
            id=new_id,
        )
        row = cur.fetchone()
        entry = _serialize_csv_entry(row)
...
@app.get('/table/csv_entries')
def table_csv_entries():
    try:
        conn = _oracle_connect()
    except Exception as e:
        return jsonify({ 'error': f'Oracle connect failed: {e}' }), 500
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT ID, FILE_NAME, STORED_PATH, ORIGINAL_NAME, CREATED_AT FROM VEDA_WORKSHEET_CSVS ORDER BY CREATED_AT DESC"
        )
        rows = [_serialize_csv_entry(row) for row in cur.fetchall()]
...
@app.get('/table/csv_entries/<int:entry_id>')
def table_csv_entry(entry_id):
    try:
        conn = _oracle_connect()
    except Exception as e:
        return jsonify({ 'error': f'Oracle connect failed: {e}' }), 500
