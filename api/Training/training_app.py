...
if "_syn_settings" not in st.session_state:
    st.session_state._syn_settings = {
        "workers": max(2, (_os_for_syn.cpu_count() or 4) // 2),
        "use_processes": False,  # default to threads for stability; enable if stable
        "max_variants": 20,
...
with st.sidebar:
    st.markdown("### Synonym Generation Settings")
    _c1 = st.number_input(
        "Workers (CPU cores)", min_value=1, max_value=256,
        value=int(st.session_state._syn_settings["workers"]), step=1,
        help="Parallel workers. With processes, pick ~cores/2 to cores."
    )
    _c2 = st.toggle(
        "Use processes (safer on CPU)", value=st.session_state._syn_settings["use_processes"],
        help="Processes avoid the GIL, but on some envs can stall. If it hangs, turn this OFF."
    )
    _c3 = st.slider(
    ...
    st.session_state._syn_settings.update({
        "workers": int(_c1),
        "use_processes": bool(_c2),
        "max_variants": int(_c3),
        "semantic_filter": bool(_c4),
        "batch_questions": int(_c5),
        "insert_batch": int(_c6),
    })
...
icon_path = "../../client/src/icons/aimesh.svg"          # left round logo (match HeaderBar)
...
st.markdown("""
    <style>
        /* Tighten overall top padding to reduce header gap */
        ...
        .circle-label {
            ...
            box-shadow: 0 0 10px rgba(0,0,0,0.25), 0 0 18px rgba(14,99,156,0.45);
            display: flex;
            align-items: center;
            justify-content: center;
            margin-right: 14px;
        }
        .ring-overlay { position: absolute; inset: 0; border-radius: 50%; padding: 2px;
        ...
        .brand-accent { height: 3px; margin-top: 4px; border-radius: 6px; width: 100%; max-width: 160px;
            background: linear-gradient(90deg, rgba(255,255,255,0.75) 0%, #7ad3ff 35%, #6aa8ff 70%, rgba(255,255,255,0.75) 100%);
            box-shadow: 0 2px 8px rgba(0,0,0,0.25), 0 0 10px rgba(122,211,255,0.35); }
        ...
        /* Reduce default top margins for the main headings to close gaps */
        h1, h2, h3 { margin-top: 0.4rem !important; }
    </style>
""", unsafe_allow_html=True)

# ---------- Header HTML (left: logo+brand, right: optional icon) ----------
st.markdown(
    f"""
...
st.markdown("<h1 style='text-align: left; font-size: 1.8rem;'>🧠 NLP Training & Evaluation Manager</h1>", unsafe_allow_html=True)
                ...
                if not tables:
                    ...
                    if not q_list:
                        ...
                        cur.close()
                                                ...
                                                # Generate synonyms (reads settings from sidebar, robust & timed)
                        import os, time
                        cfg = st.session_state._syn_settings
                        workers         = int(cfg["workers"])
                        use_processes   = bool(cfg["use_processes"])
                        max_variants    = int(cfg["max_variants"])
                        semantic_filter = bool(cfg["semantic_filter"])
                        batch_questions = int(cfg["batch_questions"])
                        ...
                        with st.spinner("Generating synonyms..."):
                            ...
                            if total_q == 0:
                                ...
                                for chunk in _chunked(list(zip(ids, qs)), batch_questions):
                                    ...
                                    wave_log.markdown(f"**Wave {wave_idx}** — {len(batch_qs)} questions (use_processes={use_processes}, workers={workers})")
                                    ...
                                    try:
                                        ...
                                        if use_processes:
                                            st.warning(f"Process pool failed ({_proc_err}). Falling back to threads for this run.")
                                            results = generate_synonyms_bulk(
                                                batch_qs,
                                                max_variants=max_variants,
                                                workers=workers,
                                    ...
                                    wave_log.markdown(f"**Wave {wave_idx} done** — generate: **{gen_ms:.0f} ms**, insert: **{ins_ms:.0f} ms**, records: **{len(records)}**")
