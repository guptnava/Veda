...
COMPREHENSIVE_TEMPLATES = {
    ...
    "select": [
        {
            "nl_patterns": [
                "Show all records from {qualified_name}",
            ],
            "sql_template": "SELECT * FROM {schema_name}.{table_name}"
        },
        {
            "nl_patterns": [
                "Show me the {qualified_columns} from {qualified_name}",
            ],
    ...
    "count_and_aggregate": [
        {
            "nl_patterns": [
                "How many records are in {qualified_name}?",
            ],
            "sql_template": "SELECT COUNT(*) FROM {schema_name}.{table_name}"
        },
        {
            "nl_patterns": [
                "How many unique {col_qualified_name} values are there in {qualified_name}?",
            ],
        ...
        {
            "nl_patterns": [
                "What is the total of {aggregate_qualified_column} in {qualified_name}?",
            ],
        ...
        {
            "nl_patterns": [
                "What is the average {aggregate_qualified_column} in {qualified_name}?",
            ],
        ...
        {
            "nl_patterns": [
                "What is the maximum {aggregate_qualified_column} in {qualified_name}?",
            ],
    ...
    "distinct": [
        {
            "nl_patterns": [
                "Show the unique values of {qualified_column} in {qualified_name}",
            ],
    ...
    "filter": [
        {
            "nl_patterns": [
                "Show all {qualified_name} where {col_qualified_name} is {qualified_value}",
            ],
            "sql_template": "SELECT * FROM {schema_name}.{table_name} WHERE {column} = {value}",
        },
        {
            "nl_patterns": [
                "Find {qualified_name} where {col_qualified_name} is greater than {qualified_value}",
            ],
            "sql_template": "SELECT * FROM {schema_name}.{table_name} WHERE {column} > {value}",
            "requires_numeric": True
        },
        {
            "nl_patterns": [
                "Find records in {qualified_name} where {col_qualified_name} starts with {qualified_value}",
            ],
        ...
        {
            "nl_patterns": [
                "Find records in {qualified_name} where {col_qualified_name} contains {qualified_value}",
            ],
            "sql_template": "SELECT * FROM {schema_name}.{table_name} WHERE {column} LIKE '%{value}%'",
            "requires_string": True
        },
    ...
    "group_by": [
        {
            "nl_patterns": [
                "Count {qualified_name} grouped by {group_column_qualified}",
            ],
        ...
        {
            "nl_patterns": [
                "What is the total of {aggregate_column_qualified} for each {group_column_qualified} in {qualified_name}?",
            ],
        ...
        {
            "nl_patterns": [
                "What is the total of {aggregate_column_qualified} for each {group_column_qualified} in {qualified_name} with the total above {qualified_value}?",
            ],
    ...
    "order_by": [
        {
            "nl_patterns": [
                "Show the top {limit} records from {qualified_name} by {column_qualified}",
            ],
            "sql_template": "SELECT * FROM {schema_name}.{table_name} ORDER BY {column} DESC FETCH FIRST {limit} ROWS ONLY"
        },
         {
            "nl_patterns": [
                "List all {qualified_name} and order by {column_qualified}",
            ],
    ...
    #     {
    #         "nl_patterns": [
    #             "Show {t1_column_qualified} and {t2_column_qualified} by joining {t1_name} and {t2_name}",
    #         ],
...
def generate_questions(tables: list, sample_values: dict = None) -> list[dict]:
    ...
    for table in tables:
        ...
        date_cols = [c for c in table["columns"] if c["type"] == "DATE"] # Keep as dictionaries
                ...
                try:
                    ...
                    if template_group in ["select", "order_by"]:
                        ...
                        if "columns" in sql_template:
                             for i in range(1, min(len(all_cols) + 1, 4)):  # Generate queries for 1, 2, or 3 columns
                                 columns_list = []
                                 qualified_columns_list = []
                                 for c in random.sample(all_cols, i):
                                    columns_list.append(c["name"])
                                    qualified_columns_list.append(c["col_qualified_name"])

                                 params["columns"] = ", ".join(columns_list)
                                 params["qualified_columns"] = ", ".join(qualified_columns_list)

                                 # params["columns"] = ", ".join([c["name"] for c in random.sample(all_cols, i)])
                                 # params["qualified_columns"] = ", ".join([c["col_qualified_name"] for c in random.sample(all_cols, i)])
                         ...
                         # Iterate through all string and date columns for distinct
                         distinct_cols = string_cols + date_cols
                         for col_info in distinct_cols:
                            params["column"] = col_info["name"]
                            params["qualified_column"] = col_info["col_qualified_name"]
                            nl_question = random.choice(nl_patterns).format(**params)
                            sql_query = sql_template.format(**params)
                            all_questions.append({
                                     "schema_name": schema,
                    ...
                    if template_group == "count_and_aggregate":
                        if "requires_numeric" in template_data:
                            ...
                            for col_info in valid_cols:
                                ...
                                params["aggregate_qualified_column"] = col_info["col_qualified_name"]  # Get the qualified name
                                nl_question = random.choice(nl_patterns).format(**params)
                                sql_query = sql_template.format(**params)
                                all_questions.append({
                                     "schema_name": schema,
                            ...
                            params["col_qualified_name"] = random.choice(pk_cols)["col_qualified_name"] if pk_cols else random.choice(all_cols)["col_qualified_name"]  # Get the name
                    ...
                    if template_group == "filter":
                        ...
                        if "requires_numeric" in template_data:
                            ...
                            applicable_cols = [c for c in table["columns"] if c["type"] in ["VARCHAR2", "CHAR"]]
                             ...
                             # Exclude PK and FK columns for SELECT * filters
                             applicable_cols = [c for c in table["columns"] if c["sql_type"] not in ["PK", "FK"]]
                        ...
                        for col_for_value in applicable_cols:
                            ...
                            params["col_qualified_name"] = col_for_value["col_qualified_name"]

                            # Always use a placeholder for the value
                            params["value"] = "{" + col_for_value["name"] + "}"
                            params["qualified_value"] = "{" + col_for_value["col_qualified_name"] + "}"
                    ...
                    if template_group == "group_by":
                        ...
                        for group_col_info in valid_group_cols:
                            ...
                            params["group_column_qualified"] = group_col_info["col_qualified_name"]
                            if "requires_numeric" in template_data:
                                # Avoid aggregating on ID columns (PK or FK)
                                ...
                                for agg_col_info in valid_agg_cols:
                                     ...
                                     params["aggregate_column_qualified"] = agg_col_info["col_qualified_name"]  # Get the qualified name
                                     if "value" in sql_template:
                                         # Always use a placeholder for the value
                                         params["value"] = "{AGGREGATE_VALUE}"
                                         params["qualified_value"] = "{AGGREGATE_QUALIFIED_VALUE}"
                                     nl_question = random.choice(nl_patterns).format(**params)
                                     sql_query = sql_template.format(**params)
                                     all_questions.append({
                                     "schema_name": schema,
                    ...
                    if template_group == "order_by":
                         ...
                         for col_info in all_cols:
                            params["column"] = col_info["name"]
                            params["column_qualified"] = col_info["col_qualified_name"]
                         ...
                         for join_info in joinables:
                             ...
                             for t1_col_info in all_cols:
                                 for t2_col_info in t2_cols_info:
                                     ...
                                     params["t1_column_qualified"] = t1_col_info["col_qualified_name"]
                                     params["t2_column"] = t2_col_info["name"]
                                     params["t2_column_qualified"] = t2_col_info["col_qualified_name"]
                                     nl_question = random.choice(nl_patterns).format(**params)
                                     sql_query = sql_template.format(**params)
                                     all_questions.append({
                                     "schema_name": schema,
...
if __name__ == '__main__':
    ...
    mock_schema = [{"schema_name": "RISKINTEGOV2", 
                    "table_name": "SALES", 
                    "qualified_name": "QUALIFIED SALES",
                    "columns": [{"name": "PRODUCT_ID", "type": "NUMBER", "sql_type": "FK", "col_qualified_name":"PRODUCT IDENTIFIER" },
                                {"name": "CUSTOMER_ID", "type": "NUMBER", "sql_type": "FK", "col_qualified_name":"CUSTOMER IDENTIFIER"},
                                {"name": "SALE_ID", "type": "NUMBER", "sql_type": "PK", "col_qualified_name":"SALE IDENTIFIER"},
                                {"name": "UNIT_PRICE", "type": "NUMBER", "sql_type": "COL", "col_qualified_name":"UNIT PRICE"},
                                {"name": "QUANTITY", "type": "NUMBER", "sql_type": "COL", "col_qualified_name":"QUANTITY IDENTIFIER"},
                                {"name": "SALE_DATE", "type": "DATE", "sql_type": "COL", "col_qualified_name":"SALE DATE"},
                                {"name": "REGION", "type": "VARCHAR2", "sql_type": "COL", "col_qualified_name":"REGION IDENTIFIER"},
                                {"name": "TOTAL_AMOUNT", "type": "NUMBER", "sql_type": "COL", "col_qualified_name":"TOTAL AMOUNT"}
                                ]}]
