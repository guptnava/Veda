...
def ensure_tables(conn):
    ...
    BEGIN
        ...
        CREATE TABLE NL2SQL_SCHEMA (
            ...
            qualified_table_name VARCHAR2(1024),
            qualified_column_name VARCHAR2(1024),
            PRIMARY KEY (id)
        )';
    EXCEPTION
        WHEN OTHERS THEN
...
def insert_schema(conn, schema_owner: str):
    ...
    print("schema_user=====", schema_owner)
    ...
    cur.execute("""
        SELECT
            ...
            column_name  qualified_column_name
        FROM
            all_tab_columns
        WHERE table_name in ('SALES', 'EMPLOYEES') and
            data_type NOT IN ( 'CLOB', 'BLOB', 'NCLOB', 'BFILE' ) and owner=:owner 
    ...
    # Insert into table
    cur.executemany(
        "INSERT INTO NL2SQL_SCHEMA (schema_name, table_name, column_name, data_type, sql_type, qualified_table_name , qualified_column_name) VALUES (:1,:2,:3,:4,:5,:6,:7)",
        rows
    )
    conn.commit()
...
def fetch_schema_from_db(conn):
   #return pd.read_sql("SELECT a.* , b.qualified_name , c.qualified_name COL_QUALIFIED_NAME FROM NL2SQL_SCHEMA a , mds_table_meta b, mds_column_meta c where a.table_name=b.table_name and a.table_name=c.table_name", conn)
