                ...
                if not tables:
                    ...
                    if not q_list:
                        ...
                        cur.close()
                                                ...
                                                # Generate synonyms (fast, parallel, batched inserts)
                        with st.spinner("Generating synonyms..."):
                            # Prepare IDs/questions
                            ids, qs = [], []
                            ...
                            if total_q == 0:
                                ...
                                # Tuning knobs (adjust as needed for your server)
                                ...
                                for chunk in _chunked(list(zip(ids, qs)), batch_questions):
                                    ...
                                    # Build tuples and insert via DataFrame batches
                                    records = []
                                    ...
                                    for rec_chunk in _chunked(records, insert_batch):
                                        ...
                                        insert_synonyms(conn, df_chunk)  # commits inside per signature
                                        inserted_total += len(rec_chunk)
                        
                                    processed_q += len(batch_qs)
                                    progress.progress(min(1.0, processed_q / total_q), text=f"Processed {processed_q}/{total_q} questions • Inserted {inserted_total} synonyms")
