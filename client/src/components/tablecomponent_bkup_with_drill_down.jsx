...
const FormatPicker = ({ headers, numericCols, rules, setRules, useFixed = true, fixedTop = 64, fixedRight = 10, fixedLeft = null }) => {
  ...
  return (
    <div style={{ position: useFixed ? 'fixed' : 'absolute', top: useFixed ? `${fixedTop}px` : '110%', ...(fixedLeft != null ? { left: `${fixedLeft}px` } : { right: useFixed ? `${fixedRight}px` : 0 }), backgroundColor: "#252526", border: "1px solid #444", borderRadius: 6, padding: 10, zIndex: 1000, minWidth: 320, maxWidth: 420, boxShadow: "0 4px 12px rgba(0,0,0,0.5)" }}>
...
const DerivedPicker = ({ baseHeaders, derivedCols, setDerivedCols, useFixed = true, fixedTop = 64, fixedRight = 10, fixedLeft = null }) => {
  ...
  return (
    <div style={{ position: useFixed ? 'fixed' : 'absolute', top: useFixed ? `${fixedTop}px` : '110%', ...(fixedLeft != null ? { left: `${fixedLeft}px` } : { right: useFixed ? `${fixedRight}px` : 0 }), backgroundColor: "#252526", border: "1px solid #444", borderRadius: 6, padding: 10, zIndex: 1000, minWidth: 360, maxWidth: 520, boxShadow: "0 4px 12px rgba(0,0,0,0.5)" }}>
...
const TableComponent = React.memo(({ data, initialPageSize = 10, initialFontSize = 11, buttonsDisabled = false, buttonPermissions, perfOptions, previewOptions, exportContext, totalRows, virtualizeOnMaximize = true, virtualRowHeight = 28, onMaximize }) => {
  ...
  const pivotedData = React.useMemo(() => {
    ...
    const computeAggs = (arr) => {
      ...
      for (const m of measures) {
        ...
        for (const f of funcs) {
          const label = `${m} (${f})`;
          switch (f) {
            case 'sum': out[label] = vals.filter(v => typeof v === 'number').reduce((a,b)=>a+b,0); break;
            case 'avg': {
    ...
    const grandTotals = computeAggs(sortedData);
    ...
    const t1 = (typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now());
    if (isDev) console.log(`[Table] Pivot computed in ${Math.round(t1 - t0)}ms (rows=${sortedData.length}, levels=${pivotConfig.columns.length}, labels=${labels.length})`);
    return rows;
  }, [isPivotView, pivotConfig, sortedData]);

  // pagination (last step)
  const sourceRows = isPivotView ? pivotedData : sortedData;
  const totalPages = Math.ceil(sourceRows.length / pageSize) || 1;
  const paginatedData = React.useMemo(() => {
    const start = (currentPage - 1) * pageSize;
    return sourceRows.slice(start, start + pageSize);
  }, [sourceRows, currentPage, pageSize]);
  ...
  const [toolbarBottom, setToolbarBottom] = useState(64);
  const [anchors, setAnchors] = useState({}); // { key: { top, left, width } }
  const calcAnchor = (e, panelWidth = 400) => {
    try {
      const rect = e.currentTarget.getBoundingClientRect();
      const top = rect.bottom + 6 + (window.scrollY || window.pageYOffset || 0);
      let left = rect.left + (window.scrollX || window.pageXOffset || 0);
      const minLeft = (window.scrollX || 0) + 8;
      const maxLeft = (window.scrollX || 0) + window.innerWidth - panelWidth - 8;
      left = Math.max(minLeft, Math.min(left, maxLeft));
      return { top, left, width: panelWidth };
    } catch { return { top: toolbarBottom, left: 10, width: panelWidth }; }
  };
  useEffect(() => {
    const update = () => {
      try {
        ...
        setToolbarBottom(Math.max(48, Math.min(top, 2000)));
      } catch {}
    };
    update();
    window.addEventListener('resize', update);
  ...
  return (
    <div className={`table-container ${allDisabled ? 'disable-buttons' : ''}`} style={containerStyle} aria-disabled={allDisabled}>
      ...
      <div ref={toolbarRef} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: 6, border: '1px solid #333', borderRadius: 6, background: '#1f1f1f', marginBottom: 8, flexWrap: 'wrap' }}>
        {/* Column Picker */}
        <ToolbarButton
          icon={ICON_COLUMNS}
          alt="Columns"
          ...
          onClick={(e) => { setShowColumnPicker((p) => !p); setAnchors(prev => ({ ...prev, columns: calcAnchor(e, 260) })); }}
        />
        {/* Filters */}
        <ToolbarButton
          icon={ICON_FILTERS}
          ...
          onClick={(e) => { setShowFilterPicker((p) => !p); setAnchors(prev => ({ ...prev, filters: calcAnchor(e, 560) })); }}
        />
        {/* Advanced Filters */}
        <ToolbarButton
          icon={ICON_ADVANCED}
          ...
          onClick={(e) => { setShowAdvancedPicker((p) => !p); setAnchors(prev => ({ ...prev, advanced: calcAnchor(e, 640) })); }}
        />
        {/* Pivot Options */}
        <ToolbarButton
          icon={ICON_PIVOT}
          ...
          onClick={(e) => { setShowPivotPicker((p) => !p); setAnchors(prev => ({ ...prev, pivot: calcAnchor(e, 560) })); }}
        />
        {/* Derived Columns */}
        <ToolbarButton
          icon={ICON_DERIVED}
          ...
          onClick={(e) => { setShowDerivedPicker((p) => !p); setAnchors(prev => ({ ...prev, derived: calcAnchor(e, 520) })); }}
        />
        ...
        <ToolbarButton
          ...
          onClick={(e) => { setShowFormatPicker((p) => !p); setAnchors(prev => ({ ...prev, condFormat: calcAnchor(e, 420) })); }}
        />
        {/* Number/Date Formats */}
        <ToolbarButton
          icon={ICON_FORMATS}
          ...
          onClick={(e) => { setShowFormatsPanel((p) => !p); setAnchors(prev => ({ ...prev, formats: calcAnchor(e, 560) })); }}
        />
        {/* Chart Config */}
        <ToolbarButton
          icon={ICON_CHART}
        ...
        <input
          ...
          style={{ flex: 1, minWidth: 160, maxWidth: 360, padding: 6, borderRadius: 4, border: '1px solid #444', background: '#1e1e1e', color: '#d4d4d4' }}
        />
        ...
        <label style={{ color: '#aaa', display: 'inline-flex', alignItems: 'center', gap: 4 }} title="Search only visible columns">
          <input type="checkbox" checked={searchVisibleOnly} onChange={(e) => setSearchVisibleOnly(e.target.checked)} /> visible
        </label>

        {/* Font size */}
        <label style={{ color: '#aaa', display: 'inline-flex', alignItems: 'center', gap: 4 }} title="Font size">
          A
          <input type="number" min="10" max="24" value={fontSize} onChange={(e) => setFontSize(Number(e.target.value))} style={{ width: 56, padding: '2px', background: '#1e1e1e', border: '1px solid #444', color: '#d4d4d4', borderRadius: 4 }} />
        </label>

        {/* Freeze columns */}
        <label style={{ color: '#aaa', display: 'inline-flex', alignItems: 'center', gap: 4 }} title="Freeze left columns">
          Freeze
          <input type="number" min="0" max={displayColumns.length} value={freezeCount} onChange={(e) => setFreezeCount(Math.max(0, Math.min(displayColumns.length, Number(e.target.value))))} style={{ width: 56, padding: '2px', background: '#1e1e1e', border: '1px solid #444', color: '#d4d4d4', borderRadius: 4 }} />
        </label>
      </div>
      {/* Controls */}
      <div className="sec-controls" style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "8px", alignItems: "center" }}>
        {/* Search, font, freeze moved to toolbar */}

        {/* Column picker (trigger moved to toolbar) */}
        <div style={{ position: "relative", ...(allDisabled || !perm.columns ? disabledStyle : {}) }} ref={pickerRef}>
          {showColumnPicker && (
            <div style={{ position: "absolute", top: "110%", right: 0, backgroundColor: "#252526", border: "1px solid #444", borderRadius: "6px", padding: "8px", zIndex: 20, display: "flex", flexDirection: "column", gap: "6px", boxShadow: "0 4px 12px rgba(0,0,0,0.5)", minWidth: "220px", maxHeight: "300px", overflowY: "auto" }}>
    ...
    <div style={{ position: 'fixed', top: 0, right: 0, height: '100%', width: '50%', maxWidth: 640, background: '#111', borderLeft: '1px solid #333', zIndex: 100, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ overflow: 'auto', padding: 8 }}>
        ...
        <div style={{ position: 'relative', ...(allDisabled || !perm.filters ? disabledStyle : {}) }} ref={filterRef}>
          {/* Filters trigger moved to toolbar (icon). */}
          {showFilterPicker && (
            <div style={{ position: 'fixed', top: `${(anchors.filters?.top ?? toolbarBottom)}px`, left: `${anchors.filters?.left ?? 10}px`, background: '#252526', border: '1px solid #444', borderRadius: 6, padding: 8, zIndex: 1000, minWidth: 420, maxWidth: 560, boxShadow: '0 4px 12px rgba(0,0,0,0.5)' }}>
        ...
        <div style={{ position: "relative", ...(allDisabled || !perm.pivot ? disabledStyle : {}) }} ref={pivotRef}>
          {showPivotPicker && (
            <div style={{ position: 'fixed', top: `${(anchors.pivot?.top ?? toolbarBottom)}px`, left: `${anchors.pivot?.left ?? 10}px`, backgroundColor: "#252526", border: "1px solid #444", borderRadius: "6px", padding: "8px", zIndex: 1000, display: "flex", flexDirection: "column", gap: "6px", boxShadow: "0 4px 12px rgba(0,0,0,0.5)", minWidth: "260px" }}>
              ...
              {/* Toggle pivot view */}
              <button onClick={() => setIsPivotView((prev) => !prev)} style={{ marginTop: 8, padding: 8, border: "none", borderRadius: 4, background: isPivotView ? "#a33" : "#0e639c", color: "white", cursor: "pointer" }}>
                {isPivotView ? "Switch to Normal View" : "Switch to Pivot View"}
              </button>
            </div>
          )}
        </div>
        ...
        <div style={{ position: 'relative', ...(allDisabled || !perm.derived ? disabledStyle : {}) }} ref={derivedRef}>
          {showDerivedPicker && (
            <DerivedPicker
              ...
              useFixed={true}
              fixedTop={anchors.derived?.top ?? toolbarBottom}
              fixedLeft={anchors.derived?.left ?? 10}
            />
          )}
        </div>
        ...
        <div style={{ position: 'relative', ...(allDisabled || !perm.formatting ? disabledStyle : {}) }} ref={formatRef}>
          {showFormatPicker && (
            <FormatPicker headers={headers} numericCols={numericCols} rules={condRules} setRules={setCondRules} useFixed={true} fixedTop={anchors.condFormat?.top ?? toolbarBottom} fixedLeft={anchors.condFormat?.left ?? 10} />
          )}
        </div>
        ...
        <div style={{ position: 'relative' }} ref={formatsRef}>
          {showFormatsPanel && (
            <div style={{ position: 'fixed', top: `${(anchors.formats?.top ?? toolbarBottom)}px`, left: `${anchors.formats?.left ?? 10}px`, background: '#252526', border: '1px solid #444', borderRadius: 6, padding: 8, zIndex: 1000, minWidth: 420, maxWidth: 560, maxHeight: 260, overflow: 'auto', boxShadow: '0 4px 12px rgba(0,0,0,0.5)' }}>
        ...
        <div style={{ position: 'relative' }} ref={advancedRef}>
          {/* Advanced trigger moved to toolbar (icon). */}
          {showAdvancedPicker && (
            <div style={{ position: 'fixed', top: `${(anchors.advanced?.top ?? toolbarBottom)}px`, left: `${anchors.advanced?.left ?? 10}px`, background: '#252526', border: '1px solid #444', borderRadius: 6, padding: 8, zIndex: 1000, minWidth: 520, maxWidth: 640, boxShadow: '0 4px 12px rgba(0,0,0,0.5)' }}>
      ...
      <Suspense fallback={<div style={{ color: '#aaa', margin: '8px 0' }}>Loading charts…</div>}>
        <ChartPanel
          headers={headers}
          rows={sortedData}
          disabled={isPivotView || allDisabled || !perm.chart}
              ...
              >
                ...
                {!isPivotView && headerFilterOpenCol === header && (
                  <div ref={el => (headerFilterMenuRef.current = el)} style={{ position: 'absolute', top: '100%', marginTop: 4, background: '#252526', border: '1px solid #444', borderRadius: 6, padding: 8, zIndex: 50, minWidth: 220, maxWidth: 320, maxHeight: 260, overflow: 'auto', boxShadow: '0 4px 12px rgba(0,0,0,0.5)', ...(idx === 0 ? { left: 0 } : { right: 0 }), ...(allDisabled || !perm.headerMenu ? disabledStyle : {}) }} onClick={(e) => e.stopPropagation()}>
                    ...
                    {(() => {
                      // Show distinct values from a sampled, filtered dataset to stay performant on very large tables.
                      const maxScan = Math.max(1, Math.min(5000, (perfOptions && perfOptions.maxScan) || 5000));
                      const maxDistinct = Math.max(1, Math.min(50, (perfOptions && perfOptions.maxDistinct) || 50));
                      ...
                      const distinct = Array.from(set).sort((a,b) => a.localeCompare(b));
      ...
      {(() => {
        ...
        if (effectiveTotal > baseCount) parts.push(`loaded ${baseCount} in UI`);
